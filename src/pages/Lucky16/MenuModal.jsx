...
function MenuModal({ data,setToggleMenu }) {
    return (
        <div className="fixed inset-0 z-50 flex justify-center  bg-black bg-opacity-50">
            <div className=" text-white absolute z-50 p-2 top-10 w-[310px] border-gray border-[1px] bg-blackAviator4 shadow-lg rounded-lg ">
                        ...
                        <tbody>
                            {data?.length > 0 ? data.map((item, i) => (
                                <tr key={i} className="text-[12px]">
                                    ...
                                    <td className="py-1 text-center">{item?.period_no}</td>
